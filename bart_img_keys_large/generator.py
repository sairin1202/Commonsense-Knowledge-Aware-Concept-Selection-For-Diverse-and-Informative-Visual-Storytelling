    ...
    def _decode_one(
        ...
        if self.incremental_states is not None:
            decoder_out = list(model._interface.model.forward_decoder(
                tokens, encoder_out=encoder_out, incremental_state=self.incremental_states[model._interface.model],
            ))
        else:
            decoder_out = list(model._interface.model.forward_decoder(tokens, encoder_out=encoder_out))
...
def forward_embedding(self, src_tokens, device_embed_tokens):
    ...
    embed = self.embed_scale * self.embed_tokens(
        src_tokens.to(device_embed_tokens)).to('cuda')
